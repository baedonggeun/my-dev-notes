...
### 상속 제어 (`abstract` / `virtual` / `override` / `sealed` / `new`)
...
| tuple pattern | 튜플 매칭 (`#C#8`) | `(atk, def) switch { (Fire, Wind) => 1.2f, _ => 1f }` |
...
### `yield return` / `yield break` — 이터레이터
