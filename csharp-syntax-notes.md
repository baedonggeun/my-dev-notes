...
> 각 항목 형식: **핵심 개념 먼저** (mental model) → **비교표** → **전체 예시** (정의+호출 양쪽) → **함정** (왜 함정인지 이유 포함) → **대표 용도 요약**
...
**핵심 개념 먼저**: C#의 기본 접근자는 `private` — 의도적으로 공개할 것만 명시하는 방향이 원칙.
...
**핵심 개념 먼저**: `?`가 값 타입에 붙는 것과 참조 타입에 붙는 것은 완전히 다른 메커니즘이다.
...
**핵심 개념 먼저**: C# 버전마다 `is` 한 줄이 표현하는 범위가 확장되었다.
- C#6: 타입 검사만
- C#7: 검사 + 변수 바인딩 동시
...
**핵심 개념 먼저**: N개의 독립적인 if-else 중첩을 N×M 매트릭스로 펼쳐 가독성을 높인다. 조합 수가 많을수록 중첩 if보다 유리하다.
...
**핵심 개념 먼저**: 제네릭이 없으면 같은 로직을 타입별로 중복 작성하거나, `object`를 써서 박싱 비용과 캐스트 실패 위험을 감수해야 한다. 제네릭은 컴파일 타임에 타입을 확정해 두 문제를 동시에 해결.
```csharp
...
**핵심 개념 먼저**: 세 키워드 모두 런타임 오버헤드 없이 컴파일 타임에 값이 결정된다.
...
**핵심 개념 먼저**: 일반 `enum`은 "하나의 상태만", `[Flags] enum`은 "여러 상태의 조합"을 표현한다.
