...
**한 줄 요약**
GameObject/씬에 독립적으로 존재하는 데이터 에셋. 씬 간 공유 + Inspector 편집. "설정·데이터·풀의 SOT"를 코드 대신 에셋으로 관리.
...
**메타**
- 종속성: `#Unity전용` `#필수`
- 관련 노트: [[unity-feature-notes]] #4 SerializeField/Serializable, #42 SerializeReference, #22 Resources.Load
- 첫 도출: CasualStrategy (2026-05-15) — 전 도메인에 걸쳐 사용
- 적용 사례: WeaponDataSO, EnemyPresetSO, SynergyRuleSO, GachaPoolConfigSO 등
- 태그: `#코어` `#데이터` `#필수`
...
**한 줄 요약**
abstract/interface 필드를 ScriptableObject 인라인으로 직렬화하고 인스펙터 드롭다운으로 자식 타입 선택. `[Serializable]` 자식 클래스 + `[SerializeReference, SubclassSelector]` 필드 선언으로 폴리모픽 데이터·동작 응집.
...
**메타**
- 종속성: `#Unity전용` `#데이터` `#직렬화` (Unity 2019.3+, SubclassSelector 외부 라이브러리)
- 관련 노트: [[unity-feature-notes]] #2 ScriptableObject, #4 SerializeField/Serializable
- 첫 도출: CasualStrategy (2026-05-15) — SynergyRuleSO 폴리모픽 Effect 직렬화
- 태그: `#데이터` `#직렬화` `#에디터`
...
**한 줄 요약**
Base Animator Controller의 placeholder 스테이트를 런타임에 `aoc[key] = data.clip`으로 교체해 N개 클립 변형 재생. `animator.Update(0f)` 즉시 평가로 풀 재사용 시 첫 프레임 깜빡임 차단. AOC는 Native Object → `Destroy(aoc)` 필수.
...
**메타**
- 종속성: `#Unity전용` `#Animator`
- 관련 노트: [[unity-feature-notes]] #40 Animator Layer + Avatar Mask, #36 CanvasGroup 기반 UI Pool (풀 패턴)
- 첫 도출: CasualStrategy (2026-05-15) — 적별 모션 변형
- 태그: `#애니메이션` `#코어` `#Pool`
