...
- **# 풀노트**: 작성 기준 충족 항목의 상세 기술 (없으면 `*풀노트 없음.*` 명시)
- 풀노트 항목 제목의 번호 범위: 단일 `N.` / 연속 `N~M.` / 비연속 `N, M, P.`

### 등재 트리거
- **두 번째 만남** — 같은 패턴을 두 번 마주치면 인덱스 한 줄 추가. 자명한 항목은 인덱스로 종료, 나머지는 즉시 풀노트 작성
...
인덱스 1줄만으로 실제 구현/적용이 불충분한 항목. **"이 노트만 보고 구현할 수 있는가?"** 가 기준. 보면 즉시 이해되는 자명한 항목만 인덱스로 종료.
