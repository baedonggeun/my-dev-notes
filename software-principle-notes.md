...
**한 줄 요약**
"조기 최적화는 모든 악의 근원" — Knuth. 단, *게임의 핫패스*에서는 측정 전 사전 인지 비용이 0이므로 예방 적용이 허용.
...
- **사전 최적화 허용 (배열)**: `game-misc #12 Array.Empty<T>()` — 빈 배열을 함수 반환마다 `new T[0]`으로 할당하는 것이 자명한 낭비 → 정적 공유 배열
...
- 짝 원칙: **YAGNI**(#1) — 성능 기능도 YAGNI 대상
- 반대 원칙: **Make It Work, Make It Right, Make It Fast**(#26) — "Make It Fast"는 마지막 단계
- 검출: Unity Profiler / Memory Profiler 실측 근거. `GC.Collect` 빈도와 프레임 타임 spike가 핵심 지표

**메타**
- 종속성: `#언어독립` `#성능`
- 첫 도출: CasualStrategy (2026-05-21) — Unity Update 핫패스 NonAlloc 결정 시 기준
- 태그: `#성능` `#구현` `#코드`
...
**한 줄 요약**
메서드는 (자신의 필드 / 파라미터 / 자신이 생성한 객체 / 자신)의 메서드만 호출. `a.b.c.d()` 같은 체인 호출은 중간 레이어에 *구조 의존*을 만든다.
...
**오용 사례 / 반대 원칙**
- 짝 원칙: **Tell, Don't Ask**(#21) — Law of Demeter의 *동작 위임* 측면을 강조한 표현
...
**메타**
- 종속성: `#언어독립` `#OOP`
- 첫 도출: CasualStrategy (2026-05-21) — UI 4축 비적용 영역이 오용 사례로 등재
- 태그: `#설계` `#코드` `#결합도`
...
**한 줄 요약**
메서드는 *명령*(상태 변경, void 반환)이거나 *쿼리*(상태 조회, 값 반환) 중 하나. 단, `EndBatch` 같은 패턴은 원자성이 목적인 *의도된 위반*으로 허용.
...
- **의도된 위반**: `RunManager.EndQueueBatch()` — 배치 완료 + OnQueueChanged 발화. [[game-misc-notes]] #6에 documented
...
- 짝 원칙: **POLA**(#27) — CQS를 지키면 메서드 이름만으로 부작용 유무가 자명해져 POLA 달성
- 검출 신호: `GetX()` 내부에 `_state = ...` 또는 이벤트 발화 → 의도된 위반인지 확인. 의도된 위반은 이름을 `Apply/Execute/EndBatch`로 바꿔 명시

**메타**
- 종속성: `#언어독립` `#OOP`
- 첫 도출: CasualStrategy (2026-05-21) — EndBatch 의도된 위반 문서화
- 태그: `#설계` `#구현` `#코드`
