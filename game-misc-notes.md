...
**한 줄 요약**
OnEnable 중복 발화 시에도 초기화 로직이 1회만 실행되도록 bool 가드 + 전용 OnInitialize 메서드로 분리.
...
**메타**
- 종속성: `#게임엔진일반`
- 관련 노트: [[game-misc-notes]] #8 Service 지연 주입
- 첫 도출: CasualStrategy (2026-05-15) — UIBinderBase 공통 베이스
- 태그: `#가드` `#NRE방지` `#초기화`
...
**한 줄 요약**
`FindAnyObjectByType`을 첫 호출에만 실행하고 결과를 필드에 캐시. 이후 호출은 캐시 반환.
...
**메타**
- 종속성: `#Unity전용`
- 관련 노트: [[unity-feature-notes]] FindAnyObjectByType
- 첫 도출: CasualStrategy (2026-05-15) — TooltipManager.GetSynergyCtrl
- 태그: `#캐시` `#성능`
...
**한 줄 요약**
Canvas SortingOrder 숫자를 Manager 상수로 중앙화. 레이어 간 상대 관계를 이름으로 표현.
...
**메타**
- 종속성: `#Unity전용`
- 관련 노트: [[unity-feature-notes]] Canvas SortingOrder
- 첫 도출: CasualStrategy (2026-05-15) — UIManager 레이어 관리
- 태그: `#렌더링경계` `#UI`
...
**한 줄 요약**
정적 DragContext가 드래그 상태(데이터 + 고스트 오브젝트)를 보유. 드래그 시작 시 고스트 생성, 종료 시 파괴.
...
**메타**
- 종속성: `#게임엔진일반`
- 관련 노트: [[unity-feature-notes]] EventSystem IDragHandler
- 첫 도출: CasualStrategy (2026-05-15) — 인벤토리 슬롯 드래그
- 태그: `#UX` `#UI`
...
**한 줄 요약**
SO 참조를 SerializeField로 받고 초기화 시 dict로 캐시. SerializeField가 비어 있으면 Resources.Load로 fallback.
...
**메타**
- 종속성: `#Unity전용`
- 관련 노트: [[game-misc-notes]] #9 SO Registry + 사전 캐시 (유사 패턴 — Awake 즉시 빌드 vs SerializeField null 방어 포함)
- 첫 도출: CasualStrategy (2026-05-15) — DataManager.BuildRuleCache
- 태그: `#캐시` `#데이터`
...
**한 줄 요약**
View는 [SerializeField] 참조만 노출하는 dumb holder. Controller/Binder가 View를 참조하고 모든 로직과 이벤트를 처리.
...
**메타**
- 종속성: `#게임엔진일반`
- 관련 노트: [[design-pattern-notes]] MVC/MVP (패턴 승격 후보)
- 첫 도출: CasualStrategy (2026-05-15) — RewardPopupView / QueueSlotView 등
- 태그: `#UI`
...
**한 줄 요약**
Manager.Instance 조회를 Awake가 아닌 OnInitialize(첫 OnEnable)로 지연해 init race 회피.
...
**메타**
- 종속성: `#게임엔진일반`
- 관련 노트: [[game-misc-notes]] #1 EnsureInitialized 가드 (OnInitialize를 1회 호출하는 메커니즘)
- 첫 도출: CasualStrategy (2026-05-15) — UIBinderBase.OnInitialize
- 태그: `#초기화` `#race회피`
...
**한 줄 요약**
Manager Awake 시 SO 리스트를 dict로 빌드. 이후 모든 룩업은 O(1) dict 탐색.
...
**메타**
- 종속성: `#Unity전용`
- 관련 노트: [[game-misc-notes]] #5 SO + Lazy Dictionary Cache + Resources Fallback (유사 패턴 — Awake 즉시 빌드 vs lazy + null 방어)
- 첫 도출: CasualStrategy (2026-05-15) — DataManager.BuildRuleCache
- 태그: `#캐시` `#데이터`
...
**한 줄 요약**
PlayerPrefs 읽기/쓰기를 정적 래퍼로 캡슐화. 키 분산·오타 방지 + 필요 시 변경 이벤트 추가.
...
**메타**
- 종속성: `#Unity전용`
- 관련 노트: [[unity-feature-notes]] PlayerPrefs
- 첫 도출: CasualStrategy (2026-05-15) — GameSettings.cs
- 태그: `#매니저`
...
**한 줄 요약**
전체 화면 오버레이에 `ICanvasRaycastFilter`를 붙여, 지정 Rect 안에서만 오버레이가 레이캐스트를 차단 해제 → 클릭이 아래 UI로 통과.
...
**메타**
- 종속성: `#Unity전용`
- 관련 노트: [[unity-feature-notes]] ICanvasRaycastFilter
- 첫 도출: CasualStrategy (2026-05-15) — TutorialHighlightFilter
- 태그: `#UI` `#가드`
...
**한 줄 요약**
빈 배열이 필요할 때 `new T[0]` 대신 `System.Array.Empty<T>()`. BCL 내부 정적 캐시 반환 — heap alloc 0.
...
**메타**
- 종속성: `#언어독립`
- 관련 노트: [[csharp-syntax-notes]] Span/Memory (유사 GC 최적화 계열)
- 첫 도출: CasualStrategy (2026-05-15) — TutorialOverlayController.ApplySubAction
- 태그: `#성능` `#캐시`
